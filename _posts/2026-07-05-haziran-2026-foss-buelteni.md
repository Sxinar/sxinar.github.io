...
# Haziran 2026: Özgür ve Açık Kaynak Kodlu Yazılım (FOSS) Ekosistemi Raporu
...
* **Masaüstü:** "Hyprland Noctalia" adı verilen yeni, ultra-hafif ve Wayland tabanlı bir masaüstü seçeneği sunuldu. Ayrıca DNS-over-QUIC desteği standart hale getirildi. (Kaynak: *CachyOS Haziran 2026 Sürüm Duyurusu*)
...
* **İstismar (Exploit):** Saldırgan diskteki dosyaya dokunmaz. RAM'de çalışan `/usr/bin/su` gibi **setuid** (SUID) bayraklı ikili bir dosyanın (binary) in-memory (bellek içi) kopyasını bu 4-byte'lık yazma ile bozar ve saniyeler içinde "Root" yetkisine ulaşır. Diskte değişiklik olmadığı için geleneksel dosya bütünlüğü (File Integrity) araçları bu saldırıyı tespit edemez. Haziran ayında Rocky Linux, Ubuntu (Resolute) ve Red Hat bu açık için acil durum güncellemelerini zorunlu kılmıştır. (Kaynak: *Sysdig Analiz Raporu, Red Hat CVE Veritabanı*)
